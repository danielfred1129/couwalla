<?php
                ...
                foreach ($category as $cat) {
                    if ($cat == $category_id) {
                        ...
                        //whats Hot Coupons for category

                        $whatshot_data = array();
                        $whatshot_result = mysql_query("SELECT * FROM coupons WHERE whats_hot = '1' AND category like '%$category_id%' ");
                        while ($whatshot_row = mysql_fetch_array($whatshot_result)) {
                            ...
                            if(strtotime($whatshot_row['expiry_date']) > time()) {
                            $whatshot_data[] = array(
                                "id" => $whatshot_row["id"],
                        ...
                        if (mysql_num_rows($nearstore_result) > 0) {
                            while ($nearme_store_row = mysql_fetch_assoc($nearstore_result)) {
                                $nearme_data = array();
                                $coupon_result = mysql_query("SELECT * FROM coupons WHERE store_name like '%$nearme_store_row[id]%' AND customer_name = '$nearme_store_row[customer]' AND  category like '%$category_id%' ");
                                while ($nearme_coupon_row = mysql_fetch_assoc($coupon_result)) {
									...
									if(strtotime($nearme_coupon_row['expiry_date']) > time()) {
                                    $nearme_data[] = array(
                        ...
                        //Most popular
                        $popular_myquery = mysql_query("SELECT couponid,count(couponid) FROM my_coupons  group by(couponid) order by (count(couponid)) desc") or die("sdsdsd");
                        //$popular_data = array();

                        while ($popular_mycoupon_row = mysql_fetch_assoc($popular_myquery)) {
                            $popular_data = array();
                            $popular_coupon_result = mysql_query("SELECT * FROM coupons WHERE id = '$popular_mycoupon_row[couponid]' AND category like '%$category_id%'");

                            while ($popular_coupon_row = mysql_fetch_array($popular_coupon_result)) {
                                $popular_str1 = array();
                                $popular_store = mysql_query("SELECT storename,latitude,longitude,address,description,thumbnailimage FROM stores where id in ($popular_coupon_row[store_name])");

                                while ($popular_str = mysql_fetch_assoc($popular_store)) {
                                    $popular_str1[] = array(
								...
								if(strtotime($popular_coupon_row['expiry_date']) > time()) {
                                $popular_data[] = array(
